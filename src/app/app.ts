...
export class App {
  ...
  protected readonly isAuthPage = computed(() => this.currentUrl().startsWith('/login') || this.currentUrl().startsWith('/signup') || this.currentUrl().startsWith('/map') || this.currentUrl().startsWith('/dashboard'));
  ...
  protected readonly pricingPlans = [
    { id: 'small', name: 'Small broker', loads: 100, price: 500, description: 'For focused daily activity.' },
    { id: 'medium', name: 'Medium broker', loads: 400, price: 1000, description: 'For a growing property book.' },
    { id: 'large', name: 'Large broker', loads: 1000, price: 5000, description: 'For high-volume operations.' },
  ] as const;
  protected readonly pricingOpen = signal(false);
  protected readonly dailyLoads = signal(100);
  protected readonly selectedPlan = computed(() => this.pricingPlans.find((plan) => this.dailyLoads() <= plan.loads) ?? this.pricingPlans[2]);
  ...
  protected openPricing(): void { this.pricingOpen.set(true); }
  protected closePricing(): void { this.pricingOpen.set(false); }
  protected selectPlan(loads: number): void { this.dailyLoads.set(loads); }
